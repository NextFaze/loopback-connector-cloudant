  ...
  mo.db.find(query, function(err, result) {
    ...
    if (!options || !options.raw) {
      for (var i = 0; i < result.docs.length; i++) {
        self.fromDB(model, mo, result.docs[i]);
      }
    }
    if (filter && filter.include) {
      self._models[model].model.include(result.docs,
                                        filter.include,
                                        options, cb);
    } else {
      cb(null, result.docs);
...
Cloudant.prototype.buildSelector = function(model, mo, where) {
  var self = this;
  var query = {}; query[mo.modelView] = model;
...
Cloudant.prototype.selectModel = function(model, migrate) {
  ...
  if (!modelView) modelView = (s.modelIndex || 'loopback__model__name');
